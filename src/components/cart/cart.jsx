...
export default function ShoppingCart() {
  ...
  useEffect(() => {
    ...
    updateCartTotals(cartItems, initialQuantities);
  }, [dispatch, cartItems]);
  ...
  useEffect(() => {
    if (isClient) {
      ...
      updateCartTotals(cartItems, quantities);
    }
  }, [cartItems, isClient, quantities]);
  ...
  const updateCartTotals = (updatedCartItems, updatedQuantities) => {
    const newSubtotal = updatedCartItems.reduce(
      (acc, item) => acc + item.price * (updatedQuantities[item.id] || 1),
      0
    );
